...
\item an module implementation unit providing
...
the set containing the innermost enclosing namespace, if any,
and otherwise is the empty set.

\item If $D$ is a \grammarterm{namespace-alias-definition}, the basis is
the set containing the namespace denoted by the
...
the type-basis is the set containing $T$.
...
the declarations found are referenced.
\item
For an expression,
...
\pnum
A declaration in the global module fragment is \term{discarded} if it
is not referenced by the module unit.
\begin{note}
