...
\begin{std.txt}
  ...
  and function templates are ignored.
    ...
    \item
    \added{All namespace-scope function or function template declarations
    that are visible to qualified name lookup (\ref{basic.lookup.qual})
    at any point in the instantiation context (\ref{module.context}) of the lookup
    are visible in their respective namespaces
...
void test() {
  ...
  f(x, Z());       // ill-formed: \tcode{S::f} in module \tcode{M} not considered
                   // even though \tcode{S} is an associated namespace
  apply(x, Z());   // OK, \tcode{S::f} is visible in instantiation context, and
