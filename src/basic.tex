...
%  follows that \term{module-import-declaration}. 
   \added{\enternote
    ...
    recursively (\ref{dcl.module.export}).
   \exitnote}
   \enterexample
   \begin{codeblock}
    // Translation unit \#1
...
\begin{std.txt}
  ...
  and function templates are ignored.
    ...
    \item[---]
    \added{All declarations that are visible in the instantiation
    context (\ref{dcl.module.context}) of the lookup are visible
    even if they are not visible during an ordinary lookup,
    excluding internal linkage declarations attached to the global module.}
  \end{itemize}
\end{std.txt}
