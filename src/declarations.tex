...
\end{after}
  ...
  a synthesized \term{legacy header unit} defined as follows:
  ...
  different from every \tcode{module-name}
  ...
  \grammarterm{export-declaration}. If such a name is
  ...
  \end{itemize}
  The \grammarterm{linkage-specification} shall not be
...
\begin{after}\color{addclr}
  \alinea
  A semantic property is \term{reachable} from a program point if
  \begin{itemize}\color{addclr}
  ...
  for which all indirect \grammarterm{import-declaration}{s} appear within the
