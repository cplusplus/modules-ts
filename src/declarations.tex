...
\begin{std.txt}
  ...
  \exitnote
  If the definition of a\added{n inline} function or variable
  appears in a translation unit 
  \added{at a point where no inline declaration of it is visible}
  \removed{before its first declaration as inline},
  ...
  \added{a visible} \removed{declared} inline
...
\alinea
  ...
  \item If the declaration is
  \begin{itemize}\color{addclr}
  \item a replaceable global allocation or deallocation
  function (\stdref{new.delete.single}{21.6.2.1}, \stdref{new.delete.array}{21.6.2.2}), or
  \item a \grammarterm{namespace-declaration} with external linkage, or
  \item within a \grammarterm{linkage-specification},
...
\begin{std.txt}\color{addclr}
  ...
  \item a \grammarterm{namespace-definition} that contains an
        \grammarterm{export-declaration}, or
  \item a declaration within a legacy header unit (\ref{dcl.module.import})
  ...
  If the declaration is not within a legacy header unit,
  ...
  \begin{after}\color{addclr}
  \alinea
  An exported declaration shall not redeclare a non-exported declaration.
  \begin{example}
  \begin{codeblock}
  export module M;
  export struct S;
...
\end{after}
  ...
  \enternote
  All declarations within a legacy header unit are implicitly
  ...
  permitted within a legacy header unit despite all
...
\nonterminal{global-module-fragment}:\br
    \terminal{module} \terminal{;} declaration-seq
\end{bnf}
...
(\stdref{temp.point}{12.7.4.1}) are within the module,
...
\item If $D$ is an \grammarterm{alias-declaration}, the basis is the type-basis of its \grammarterm{defining-type-id}.
...
\alinea
A declaration from a global module fragment is \term{referenced}
by the enclosing module unit $M$ if it is
\begin{itemize}
\item
the unique result of name lookup for an
\grammarterm{unqualified-id} or \grammarterm{identifier} in $M$
or in a template instantiation whose point of instantiation is in $M$, or
\item
a function named by an expression (\ref{basic.def.odr}) in $M$
or in a template instantiation whose point of instantiation is in $M$, or
\item
a lookup result for a dependent name in $M$ (\stdref{temp.dep.res}{12.7.4})
...
\begin{std.txt}\color{addclr}
  \resetalinea[0]
  \alinea
  When declarations from a translation unit $M$ are made available
...
\begin{after}\color{addclr}
  ...
  it is directly or indirectly imported prior to that program point (\ref{dcl.module.import})
  by a path of
